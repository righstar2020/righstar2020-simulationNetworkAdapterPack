...
try:
  while True:
    ...
    for h in range(0,len(keyfields)):
        pad.addstr(0,int(h * cw),format(keyfields[h],"<"+str(cw)),curses.A_STANDOUT)
    pad.addstr(0,int(len(keyfields)*cw),format(valuefield,">"+str(cw)), curses.A_STANDOUT)
    ...
    for r in range(0, len(flows)):
      ...
      for c in range(0,len(keys)):
        pad.addstr(1+r,int(c * cw),format(keys[c],"<"+str(cw)))
      # pad.addstr(1+r,len(keys)*cw,format(value,">"+str(cw)+".6g"))
      pad.addstr(1+r,int(len(keys)*cw),format(eng_str(value,"%.3f",True),">"+str(cw)))
